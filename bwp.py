...
class BitwardenClient:
  def __init__(self, api_url="https://api.bitwarden.com",
               identity_url="https://identity.bitwarden.com", debug=False):
  ...
  def _auth_request(self, data):
    ...
    self.logger.debug(f"Sending POST request with data: {self._redact_sensitive(data)}")
    try:
      response = self.session.post(url, data=data, headers=headers)
    except Exception as e:
      self.logger.debug(f"Error during request: {e}")
      raise Exception(f"Request failed: {e}")
    self.logger.debug(f"Auth response: {response.status_code} (Content length: {len(response.text)})")
    if self.debug:
...
def main():
  ...
  parser.add_argument('--api-url', default="https://api.bitwarden.com",
                      help='Base API URL (default: %(default)s)')
  parser.add_argument('--identity-url', default="https://identity.bitwarden.com",
  ...
  decryption.add_argument('--email', required=True,
                          help='Account email (used as salt for key derivation)')
  decryption.add_argument('--password',
                          help='Account password (use env BW_PASSWORD or provide here)')
  ...
  try:
    ...
    if encrypted_vault_key:
      ...
      kdf_iterations = profile.get("kdfIterations") or profile.get("KdfIterations")
      if kdf_iterations is None:
        kdf_iterations = client.auth_data.get("KdfIterations") or client.auth_data.get("kdfIterations")
      if kdf_iterations is None:
      ...
      master_key = derive_key(master_password, master_email.encode(), kdf_iterations)
      encryption_key, mac_key, stretched_master_key = derive_stretched_keys(master_key)
      console.log(f"[blue]Using derived encryption key (HKDFExpand with info 'enc').[/blue]")
      ...
      if vault_key:
        ...
        if len(vault_key) == 64:
          ...
          console.log("[blue]Using first 32 bytes of vault key for item decryption.[/blue]")
        else:
          item_decryption_key = vault_key
        decrypted_items = []
        ciphers = sync_data.get("ciphers", [])
        for cipher in ciphers:
          decrypted_items.append(decrypt_cipher_item(cipher, item_decryption_key))
        console.print(JSON(json.dumps(decrypted_items, indent=2)))
    else:
      console.log("[yellow]Profile information for decryption not found in sync data.[/yellow]")
  except Exception as e:
